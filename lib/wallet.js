...
class Wallet {
    constructor(hostname, port, user, pass) {
        ...
        this.port = port || 34569;
        this.user = user || '';
        this.pass = pass || '';

        this._request('get_balance'); // This line is necessary in order to do the initial handshake between this wrapper and electroneum-wallet-rpc; without it, the first request to the wrapper fails (subsequent request succeed, though.)
    }
}
...
Wallet.prototype.create_wallet = function(filename, password, language) {
    ...
    let params = {
        filename: filename || 'electroneum_wallet',
        'password': password || '',
...
// transfer Electroneum to a single recipient, or a group of recipients
Wallet.prototype.transfer = function(destinations, options) {
    ...
    let params = {
        destinations: destinations,
        mixin: parseInt(options.mixin) || 1,
...
// helper function to convert Electroneum amount to atomic units
function convert(amount) {
    let number = Number(amount) * 1e2;
    // remove any decimals
